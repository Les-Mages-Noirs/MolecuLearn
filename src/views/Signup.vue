...
const submitForm = async () => {
    ...
    try {
        ...
        if (error.type == 'ConstraintViolationList') {
            useToast().error('L\'email ou le nom d\'utilisateur existe déjà');
        }
    }
...
<template>
    <section class="flex flex-col md:flex-row min-h-[calc(100vh-4rem)] items-center">
        <div
            class="bg-[url('../../public/img/OIG.png')]  hidden lg:block w-full md:w-1/2 xl:w-1/2 min-h-[calc(100vh-4rem)]">
        </div>
        <div
            ...
            <div class="w-2/3 h-100">
                ...
                <form class="mt-6" @submit.prevent="submitForm">
                    ...
                    <div class="mt-4">
                        ...
                        <input v-model="username" type="text" placeholder="Entrez un nom d'utilisateur"
                            class="w-full px-4 py-3 rounded-lg bg-gray-200 mt-2 border focus:border-primary focus:bg-white focus:outline-none"
                    ...
                    <div class="mt-4">
                        ...
                        <input v-model="confirmPassword" type="password" placeholder="Entrez à nouveau votre mot de passe"
                            class="w-full px-4 py-3 rounded-lg bg-gray-200 mt-2 border focus:border-primary focus:bg-white focus:outline-none"
                            minlength="6" required />
                    </div>
                ...
                <p class="mt-8">
                    Déjà un compte?
                    <a class="text-primary hover:text-primary font-semibold  cursor-pointer" @click="navigateToLogin">Se
                        connecter</a>
                </p>
            </div>
        </div>
